...
const wrapEmail = (html = '', { storeName = 'Unbley', storeLogoUrl = emailLogoUrl } = {}) => `
<!doctype html>
<html lang="en">
  <body style="margin:0;padding:0;background:#f3eee8;color:#2b211c;font-family:Arial,Helvetica,sans-serif;">
    <div style="display:none;max-height:0;overflow:hidden;opacity:0;">A message from ${storeName} via Unbley</div>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#ffffff;border:1px solid #e6ddd5;border-radius:14px;overflow:hidden;">
            <tr>
              <td style="background:#2b211c;padding:24px 32px;">
                <a href="${emailSiteUrl}" style="text-decoration:none;">
                  <img src="${storeLogoUrl || emailLogoUrl}" width="46" height="46" alt="${storeName}" style="display:block;border:0;border-radius:10px;object-fit:cover;" />
                </a>
                <div style="color:#f7f1eb;font-size:21px;font-weight:700;letter-spacing:.2px;margin-top:12px;">${storeName}</div>
                <div style="color:#cdbfb4;font-size:12px;margin-top:4px;">Powered by Unbley</div>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="border-top:1px solid #eee7e1;padding:20px 32px;color:#887a70;font-size:12px;line-height:1.6;">
                You are receiving this order email from ${storeName} via Unbley.<br />
...
export async function sendEmail({ to, subject, html, text, replyTo, sender = 'zizzystores', storeName, storeLogoUrl }) {
  ...
  try {
    const payload = {
      ...
      to,
      subject,
      html: wrapEmail(html || `<p style="margin:0;white-space:pre-line;">${String(text || '')}</p>`, { storeName, storeLogoUrl }),
      text,
      reply_to: replyTo
    };
